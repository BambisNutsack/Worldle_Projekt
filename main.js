...
//erstellt arrays nach Kontinent
function Länderwahl() {
  imgArr = [];
  NameArr = [];
  Z = 0;
  for (var i = 0; i < Länder.length; i++) {
    if (Länder[i].continent == Kontinent) {
...
function Autocomplete() {
  ...
  for (var i = 0; i < NameArr.length; i++) {
    dataList.innerHTML =
      dataList.innerHTML + "<option value='" + NameArr[i] + "'></option>";
  }
}
...
function TabellenErstellung() {
  if (Länge < 6 && Erraten == false && NameArr.includes(LänderEingabe.value)) {
    ...
    if (Länge == 6 && Erraten == false) {
      Erraten = true;
      Auflösung.innerHTML =
        "Es wäre <span style='color:#e5ff00; font-weight: 700;'>" +
        NameArr[num] +
        "</span> gewesen!";
    }
    ...
    cell = row.insertCell();
    cell.innerHTML =
      distance().toFixed(2) > 0 ? distance().toFixed(0) + " km" : "🎉";

    cell = row.insertCell();
    cell.innerHTML = HimmelsRichtungErmitteln();

    WeiterButton.style.visibility =
      Erraten == true || Länge == 6 ? "visible" : "hidden";
    VersuchButton.style.visibility =
      Erraten == true || Länge == 6 ? "hidden" : "visible";
    LänderEingabe.style.visibility =
      Erraten == true || Länge == 6 ? "hidden" : "visible";
    LänderEingabe.value = "";
  }
}
